...
$(document).ready(() => {
    const links = [{
            name: 'tiktok',
            link: 'https:/tiktok.com/ilovetomilkgojotits',
        },
        {
            name: 'instagram',
            link: 'https://instagram.com/fromthe3005',
        },
        {
            name: ' onlyfan leaks',
            link: 'https://discord.gg/tips',
        },
        {
            name: 'twitch',
            link: 'https://twitch.com/nf2u',
        },
    ];
    ...
    for (let i in links) {
        ...
        if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="images/2f0eb1d2_original.gif"> ');
    }
    ...
    app.titleChanger(['omg', 'fag', 'hyein is bae', 'dumb dumb']);
});
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(['authenticating...', "Granting access to <span style='font-size: 14px; color: #e87aff;'>[info]</span>..."], 30, () => {
        ...
        writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
            ...
            writeLine([`<i style='color: #FFFFFF'>≽^•⩊•^≼</i>`], 120, 500, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
